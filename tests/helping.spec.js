...
test("Caută joburi și apasă săgețile", async ({ page }) => {
  ...
  await helping.searchJobs(["Tester", "Developer", "Automation"]);
});
...
test("Link email este clickable", async ({ page }) => {
  ...
  await expect(helping.emailLink).toHaveAttribute(
    "href",
    "mailto: aocpeviitor@gmail.com",
  );
});
test("Link Declaratie este clickable", async ({ page }) => {
  ...
  await expect(helping.declaratieLink).toHaveAttribute(
    "href",
    "https://redirectioneaza.ro/asociatia-oportunitati-si-cariere/",
  );
});
